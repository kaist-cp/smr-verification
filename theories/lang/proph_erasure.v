...
Lemma erased_base_step_base_step_AllocN n v σ (l:blk) :
  (0 < n)%Z →
...
Qed.
Lemma erased_base_step_base_step_Free n l σ :
  (0 < n)%Z →
  (∀ m, is_Some (erase_heap σ.(heap) !! (l +ₗ m)) ↔ 0 ≤ m < n)%Z →
  base_steps_to_erasure_of (Free #n #l) σ #()
...
Proof.
  ...
  - by eapply erased_base_step_base_step_AllocN.
  - by eapply erased_base_step_base_step_Free.
Qed.
...
Lemma base_step_erased_prim_step_allocN σ (l:blk) n v:
  (0 < n)%Z →
  (∀ m, σ.(heap) !! (l,m) = None) →
  ∃ e2' σ2' ef',
...
Lemma base_step_erased_prim_step_free σ l n :
  (0 < n)%Z →
  (∀ m, is_Some (σ.(heap) !! (l +ₗ m)) ↔ 0 ≤ m < n)%Z →
  ∃ e2' σ2' ef', prim_step (Free #n #l) (erase_state σ) [] e2' σ2' ef'.
Proof.
  intros Hw. do 3 eexists; apply base_prim_step; econstructor; auto.
