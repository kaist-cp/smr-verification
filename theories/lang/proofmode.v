...
Lemma tac_wp_allocN Δ Δ' s E j1 j2 K v n Φ :
  (0 < n)%Z →
  MaybeIntoLaterNEnvs 1 Δ Δ' →
  (∀ (l : blk) (sz : nat), n = sz →
    match envs_app false (Esnoc (Esnoc Enil j1 (array l (DfracOwn 1) (replicate sz v))) j2 (†l…sz)) Δ' with
    | Some Δ'' =>
...
Proof.
  rewrite envs_entails_unseal=> ? ? HΔ.
  rewrite -wp_bind. eapply wand_apply; first by iApply wp_allocN.
  rewrite left_id into_laterN_env_sound; apply later_mono, forall_intro=> l.
  specialize (HΔ l (Z.to_nat n) ltac:(lia)).
  destruct (envs_app _ _ _) as [Δ''|] eqn:HΔ'; [ | contradiction ].
  rewrite envs_app_sound //; simpl.
  apply wand_intro_l.
  iIntros "[[??] HFin]". iApply HΔ. iApply "HFin". iFrame.
Qed.
Lemma tac_twp_allocN Δ s E j1 j2 K v n Φ :
  (0 < n)%Z →
  (∀ (l : blk) (sz : nat), n = sz →
    match envs_app false (Esnoc (Esnoc Enil j1 (array l (DfracOwn 1) (replicate sz v))) j2 (†l…sz)) Δ with
    | Some Δ' =>
...
Proof.
  rewrite envs_entails_unseal=> ? HΔ.
  rewrite -twp_bind. eapply wand_apply; first by iApply twp_allocN.
...
Lemma tac_wp_alloc Δ Δ' s E j1 j2 K v Φ :
  MaybeIntoLaterNEnvs 1 Δ Δ' →
  (∀ (l : blk),
    match envs_app false (Esnoc (Esnoc Enil j1 (l ↦ v)) j2 (†l…1)) Δ' with
    | Some Δ'' =>
...
Proof.
  rewrite envs_entails_unseal=> ? HΔ.
  rewrite -wp_bind. eapply wand_apply; first by iApply wp_alloc.
  rewrite left_id into_laterN_env_sound; apply later_mono, forall_intro=> l.
  specialize (HΔ l).
  destruct (envs_app _ _ _) as [Δ''|] eqn:HΔ'; [ | contradiction ].
  rewrite envs_app_sound //; simpl.
  ...
  iIntros "[[??] HFin]". iApply HΔ. iApply "HFin". iFrame.
Qed.
Lemma tac_twp_alloc Δ s E j1 j2 K v Φ :
  (∀ (l : blk),
    match envs_app false (Esnoc (Esnoc Enil j1 (l ↦ v)) j2 (†l…1)) Δ with
    | Some Δ' =>
       envs_entails Δ' (WP fill K (Val $ LitV $ LitLoc l) @ s; E [{ Φ }])
...
Proof.
  rewrite envs_entails_unseal=> HΔ.
  rewrite -twp_bind. eapply wand_apply; first by iApply twp_alloc.
  rewrite left_id. apply forall_intro=> l.
  specialize (HΔ l).
  destruct (envs_app _ _ _) as [Δ''|] eqn:HΔ'; [ | contradiction ].
  rewrite envs_app_sound //; simpl.
...
Lemma tac_wp_free Δ Δ' Δ'' Δ''' s E i1 i2 K l vl (n : Z) (n' : nat) Φ :
  n = length vl →
  MaybeIntoLaterNEnvs 1 Δ Δ' →
  envs_lookup i1 Δ' = Some (false, l ↦∗ vl)%I →
  Δ'' = envs_delete false i1 false Δ' →
  envs_lookup i2 Δ'' = Some (false, †l…n')%I →
  Δ''' = envs_delete false i2 false Δ'' →
  n' = length vl →
  envs_entails Δ''' (WP fill K (Val $ LitV LitUnit) @ s; E {{ Φ }}) →
  envs_entails Δ (WP fill K (Free (LitV n) (LitV l)) @ s; E {{ Φ }}).
Proof.
  rewrite envs_entails_unseal; intros -> ? Hlk -> Hlk2 -> -> HΔ.
  rewrite -wp_bind. eapply wand_apply; first by iApply wp_free; simpl.
  rewrite into_laterN_env_sound -!later_sep; apply later_mono.
  do 2 (rewrite envs_lookup_sound //). simpl. rewrite HΔ True_emp emp_wand -assoc.
  iIntros "($&$&$)".
Qed.
Lemma tac_twp_free Δ Δ' Δ'' s E i1 i2 K l vl (n : Z) (n' : nat) Φ :
  n = length vl →
  envs_lookup i1 Δ = Some (false, l ↦∗ vl)%I →
  Δ' = envs_delete false i1 false Δ →
  envs_lookup i2 Δ' = Some (false, †l…n')%I →
...
Proof.
  rewrite envs_entails_unseal; intros -> Hlk -> Hlk2 -> -> HΔ.
  rewrite -twp_bind. eapply wand_apply; first by iApply twp_free.
  do 2 (rewrite envs_lookup_sound //). simpl. rewrite HΔ True_emp emp_wand -assoc.
  iIntros "($&$&$)".
Qed.
...
Proof.
  ...
  rewrite -wp_bind. eapply wand_apply; first by iApply wp_load.
...
Proof.
  ...
  rewrite -wp_bind. eapply wand_apply; first by iApply wp_store.
...
Proof.
  ...
  destruct (envs_simple_replace _ _ _) as [Δ''|] eqn:HΔ''; [ | contradiction ].
  rewrite -twp_bind. eapply wand_apply; first by iApply twp_store.
...
Proof.
  rewrite envs_entails_unseal=> ???.
  destruct (envs_simple_replace _ _ _) as [Δ''|] eqn:HΔ''; [ | contradiction ].
  rewrite -wp_bind. eapply wand_apply; first by iApply wp_xchg.
  rewrite into_laterN_env_sound -later_sep envs_simple_replace_sound //; simpl.
  rewrite right_id.
...
Proof.
  rewrite envs_entails_unseal. intros.
  destruct (envs_simple_replace _ _ _) as [Δ''|] eqn:HΔ''; [ | contradiction ].
  rewrite -twp_bind. eapply wand_apply; first by iApply twp_xchg.
  rewrite envs_simple_replace_sound //; simpl.
  rewrite right_id. by apply sep_mono_r, wand_mono.
Qed.
...
Proof.
  rewrite envs_entails_unseal=> ???? Hi.
  rewrite -wp_bind. eapply wand_apply; first by iApply wp_cmpxchg_fail.
  rewrite into_laterN_env_sound -later_sep envs_lookup_split //; simpl.
  apply later_mono.
  destruct b; simpl.
  * iIntros "[#$ He]". iIntros "_". iApply Hi. iApply "He". iFrame "#".
  * by apply sep_mono_r, wand_mono.
Qed.
Lemma tac_twp_cmpxchg_fail Δ s E i K b l q v v1 v2 Φ :
  envs_lookup i Δ = Some (b, l ↦{q} v)%I →
  v ≠ v1 → vals_compare_safe v v1 →
  envs_entails Δ (WP fill K (Val $ PairV v (LitV $ LitBool false)) @ s; E [{ Φ }]) →
  envs_entails Δ (WP fill K (CmpXchg (LitV l) v1 v2) @ s; E [{ Φ }]).
Proof.
  ...
  eapply wand_apply; first by iApply twp_cmpxchg_fail.
...
Lemma tac_wp_faa Δ Δ' s E i K l z1 z2 Φ :
  ...
  match envs_simple_replace i false (Esnoc Enil i (l ↦ LitV (LitInt (FinInt (z1 + z2))))) Δ' with
...
Proof.
  rewrite envs_entails_unseal=> ???.
  destruct (envs_simple_replace _ _ _) as [Δ''|] eqn:HΔ''; [ | contradiction ].
  rewrite -wp_bind. eapply wand_apply; first by iApply (wp_faa _ _ _ z1 z2).
  rewrite into_laterN_env_sound -later_sep envs_simple_replace_sound //; simpl.
  rewrite right_id. by apply later_mono, sep_mono_r, wand_mono.
Qed.
...
Proof.
  rewrite envs_entails_unseal=> ??.
  destruct (envs_simple_replace _ _ _) as [Δ'|] eqn:HΔ'; [ | contradiction ].
  rewrite -twp_bind. eapply wand_apply; first by iApply (twp_faa _ _ _ z1 z2).
  rewrite envs_simple_replace_sound //; simpl.
  rewrite right_id. by apply sep_mono_r, wand_mono.
Qed.
End heap.
...
Tactic Notation "wp_alloc" ident(l) "as" constr(H) constr(Hf) :=
  ...
  let finish _ :=
    ...
    try ( match goal with Hsz : ?x = _ |- _ => rewrite <-(Z2Nat.id x) in Hsz; simpl in *; last lia end;
          apply Nat2Z.inj in Hsz;
          first [ cbv [Z.to_nat Pos.to_nat] in Hsz;
                  simpl in Hsz;
  ...
  | |- envs_entails _ (wp ?s ?E ?e ?Q) =>
    let process_single _ :=
        first
          [reshape_expr e ltac:(fun K e' => eapply (tac_wp_alloc _ _ _ _ Htmp Hf K))
    ...
    let process_array _ :=
        first
          [reshape_expr e ltac:(fun K e' => eapply (tac_wp_allocN _ _ _ _ Htmp Hf K))
          |fail 1 "wp_alloc: cannot find 'Alloc' in" e];
  ...
  | |- envs_entails _ (twp ?s ?E ?e ?Q) =>
    let process_single _ :=
        first
          [reshape_expr e ltac:(fun K e' => eapply (tac_twp_alloc _ _ _ Htmp Hf K))
          |fail 1 "wp_alloc: cannot find 'Alloc' in" e];
        finish ()
    in
    let process_array _ :=
        first
          [reshape_expr e ltac:(fun K e' => eapply (tac_twp_allocN _ _ _ Htmp Hf K))
          |fail 1 "wp_alloc: cannot find 'Alloc' in" e];
        [try fast_done
        |finish ()]
    in (process_single ()) || (process_array ())
...
Tactic Notation "wp_free" :=
  ...
  | |- envs_entails _ (wp ?s ?E ?e ?Q) =>
    first
      [reshape_expr e ltac:(fun K e' => eapply (tac_wp_free _ _ _ _ _ _ _ _ K))
